#include "AlgoElevator.h"
#include "ninebot_log.h"
#include "AlgoUtils.h"
#include "Quaternion.h"
#include "Vector3.h"

// Lib used for sleep function
//#include <unistd.h>
...
namespace ninebot_algo
{
	namespace elevator_simulation_algo
	{
		...
		bool AlgoElevator::init(test_params_t cParams)
		{
            ...
            use_saved_elevator_location = false;
        ...
        void AlgoElevator::startPoseRecord(std::string save_folder, int64_t save_time)
        {
            ...
            nStep = 0;
            m_folder_elevator = "/sdcard/elevator/";
            createFolder(m_folder_elevator);
            //m_state_file.open(m_folder_elevator + "state.txt");
            ...
            try {
                cv::imwrite("/sdcard/elevator/local_map_c.png", show, compression_params);
            }
		...
		bool AlgoElevator::step()
		{
            ...
            if(imgstream_en){
                ...
                {
                    ALOGD("AlgoElevator: depth wrong");
                    return false;
                }
        ...
        void AlgoElevator::safeControl(float v, float w) {
            ...
            if (m_safety_control){
                ...
                //if (m_ultrasonic_average < kCloseObstacleThres*500){
                    StampedVelocity velocity;
                    mRawDataInterface->retrieveBaseVelocity(velocity);
                    ALOGD("command current: (%f,%f), current vel: (%f,%f), ultrasonic_average = %f", v, w, velocity.vel.linear_velocity, velocity.vel.angular_velocity, m_ultrasonic_average);
                    if(v>0 && (std::abs(v/std::fmax(0.01,std::abs(w))) > 0.8 || m_ultrasonic_average < kCloseObstacleThres*400)){
                    //if(v>0 && ((std::abs(v/std::fmax(0.01,std::abs(w))) > 1.5 && velocity.vel.linear_velocity > 0) ||
                               //(m_ultrasonic_average < kCloseObstacleThres*750 && std::abs(v/std::fmax(0.01,std::abs(w))) > 0.7))){ // 0.8
                        //StampedVelocity velocity;
                        //mRawDataInterface->retrieveBaseVelocity(velocity);
        ...
        cv::Mat AlgoElevator::mapToShow(const cv::Mat & map) {
            ...
            if (elevator_scenario_state > MOVE_TO_DOOR-1) {
                StampedTwist odom;
        ...
        // Save the location of the elevator. Must press the button once in the elevator, stops using
        // the scenario with the door location
        void AlgoElevator::saveElevatorLocation() {

            StampedTwist odom;
            mRawDataInterface->retrieveOdometry(odom,-1);
            Eigen::Vector3f pos_elevator(odom.twist.pose.x, odom.twist.pose.y, odom.twist.pose.orientation);

            use_saved_elevator_location = true;

            elevator_position = pos_elevator;

		}

		// Saves the current location as the door location. Saved with the 'A' button on the robot
        void AlgoElevator::saveDoorLocation() {

            StampedTwist odom;
            mRawDataInterface->retrieveOdometry(odom,-1);
            Eigen::Vector3f pos_door(odom.twist.pose.x, odom.twist.pose.y, odom.twist.pose.orientation);
        ...
        void AlgoElevator::elevatorScenarioUpdate(){
            ...
            if(elevator_scenario_state == MOVE_TO_DOOR && use_saved_elevator_location == false){
                // set path
                ...
                double dist_elev_y = -3.5; //-3, use -2 in elevator
            ...
            // Case where the locations are saved: to avoid resetting the positions
            if(elevator_scenario_state == MOVE_TO_DOOR && use_saved_elevator_location == true) {
                StampedTwist odom;
                mRawDataInterface->retrieveOdometry(odom,-1);
                Eigen::Vector3f pos_start(odom.twist.pose.x, odom.twist.pose.y, odom.twist.pose.orientation);
                start_position = pos_start;
                ...
                temp << door_position(0), door_position(1);
                path.push_back(temp);
            ...
            // Make a search in the area of the goal, looking at obstacles to find best position and go
            ...
            // 5 door opens
            // go back to the original position (without looking at final orientation)
        ...
        void AlgoElevator::stepLocalPlanner()
        {
            ...
            // Turn around to face elevator door phase
                ...
                if(yaw < 0){
                    ...
                    if(yaw > target_angle - 3.1415/20 && yaw < target_angle + 3.1415/20){
                        this->safeControl(0,0);
                        elevator_scenario_state += 1;
                        elevatorScenarioUpdate();
                    }
		...
		void AlgoElevator::setNewElevatorGoal() {
            ...
            int width = 1 * m_local_map.rows/6; // 2* = 2 meters (1 each way) //use 1 inside real elevator
